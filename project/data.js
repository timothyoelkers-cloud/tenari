/* Seed data + helpers — globals on window */

// Deterministic PRNG so screens are stable
...
const fmtPct = (n, p = 1) => n.toFixed(p) + '%';

// Tenant data — 60 fictional org names
const ORG_NAMES = [
...
const TENANTS = ORG_NAMES.map((name, i) => {
  const r = mulberry32(100 + i);
  const subs = 1 + Math.floor(r() * 12);
  const monthly = 800 + Math.floor(r() * 240000);
  ...
  return {
    id: 'T-' + String(1000 + i),
    tenantId: [r,r,r,r,r].map(f => Math.floor(f() * 16777215).toString(16).padStart(6, '0')).join('-').slice(0, 36),
    name,
    ...
    trend,
    tags,
    color: ['#3b82f6','#22d3ee','#a3e635','#a78bfa','#f472b6','#fb923c','#34d399','#facc15'][i % 8],
  };
});

// Aggregate metrics
const TOTAL_SPEND = TENANTS.reduce((s, t) => s + t.monthly, 0);
const TOTAL_SAVINGS = TENANTS.reduce((s, t) => s + t.savings, 0);
const AVG_COMPLIANCE = TENANTS.reduce((s, t) => s + t.compliance, 0) / TENANTS.length;
const TOTAL_ALERTS = TENANTS.reduce((s, t) => s + t.alerts, 0);
const TOTAL_CRITICAL = TENANTS.reduce((s, t) => s + t.critical, 0);
const TOTAL_SUBS = TENANTS.reduce((s, t) => s + t.subs, 0);
...
// RBAC users
const USERS = [
  { id: 'U-01', name: 'Sarah Chen', email: 'sarah.chen@aegiscloud.io', role: 'Owner', tenants: 60, mfa: true, last: '2m ago', avatar: 'SC' },
  { id: 'U-02', name: 'Michael Rodriguez', email: 'mike.r@aegiscloud.io', role: 'MSP Admin', tenants: 60, mfa: true, last: '14m ago', avatar: 'MR' },
  { id: 'U-03', name: 'Jamie Kowalski', email: 'jamie.k@aegiscloud.io', role: 'Tenant Admin', tenants: 18, mfa: true, last: '1h ago', avatar: 'JK' },
  { id: 'U-04', name: 'Aisha Patel', email: 'aisha.p@aegiscloud.io', role: 'Read-Only Auditor', tenants: 60, mfa: true, last: '4h ago', avatar: 'AP' },
  ...
  { id: 'U-07', name: 'Tomás Ribeiro', email: 't.ribeiro@aegiscloud.io', role: 'Billing', tenants: 60, mfa: true, last: '32m ago', avatar: 'TR' },
  { id: 'U-08', name: 'Priya Sharma', email: 'p.sharma@aegiscloud.io', role: 'FinOps Analyst', tenants: 60, mfa: true, last: '11m ago', avatar: 'PS' },
];
...
window.AegisData = {
  TENANTS, POLICIES, SAVINGS, ALERTS, INVOICES, USERS, ROLES, AUDIT,
  SPEND_TREND, DAILY_SPEND, RESOURCE_BREAKDOWN, REGIONS,
  TOTAL_SPEND, TOTAL_SAVINGS, AVG_COMPLIANCE, TOTAL_ALERTS, TOTAL_CRITICAL, TOTAL_SUBS,
  fmtMoney, fmtMoneyK, fmtNum, fmtPct, range, round, pick,
};
